...
\begin{flushleft}
  ...
  {\fontsize{24}{30}\selectfont \textcolor{white}{`r cliente`}} \\[0.4cm]
  {\Large \textcolor{white}{`r fecha`}} \\[4cm]
...
# Listar imágenes
imagenes <- list.files("temp_maps", pattern = "(?i)\\.(png|jpg|jpeg)$", full.names = TRUE)

# Código LaTeX para insertar imágenes
imagenes_latex <- paste0("\\includegraphics[width=0.3\\textwidth]{", imagenes, "}")

# Asegurar que el número total sea múltiplo de 3
falta <- 3 - (length(imagenes_latex) %% 3)
if (falta != 3) {
  imagenes_latex <- c(imagenes_latex, rep("", falta))
}

# Dividir en filas de 3 imágenes
filas <- matrix(imagenes_latex, ncol = 3, byrow = TRUE)

# Mostrar 2 filas por hoja (6 imágenes) con salto
num_filas <- nrow(filas)
for (i in seq(1, num_filas, by = 2)) {
  bloque <- filas[i:min(i+1, num_filas), , drop = FALSE]
...
for (i in seq_along(base_nombres)) {
  ...
  # Solo mostramos "Results" en la primera iteración
  if (i == 1) {
    cat("\\newpage\n")
  ...
  # Título y descripción por centro
  ...
  # Tabla
  print(
    kable(data_tmp, format = "latex", booktabs = TRUE, align = "c") %>%
      add_header_above(c(
...
\noindent \textbf{\textcolor{turquesa}{\fontsize{16}{20}\selectfont Return Periods}}
